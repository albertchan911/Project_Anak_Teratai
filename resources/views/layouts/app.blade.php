...
<head>
    ...
    <title>@yield('title', 'Project Anak Teratai')</title>
        ...
        <div class="h-screen w-64 sidebar fixed top-0 left-0 flex flex-col justify-between">
            <div>
                <div class="flex flex-col items-center p-4">
                    <!-- Logo -->
                    <img src="{{ asset('storage/logo.png') }}" alt="Logo Kantor" class="h-20 w-auto mb-2">
                </div>
                ...
                <ul class="mt-4">
                    ...
                    @auth
                        @if(auth()->user()->role === 'superadmin' || auth()->user()->role === 'staff')
                            <li class="px-4 py-2 hover:bg-gray-700">
                                <a href="{{ route('siswa.create') }}">➕ Tambah Anak Teratai</a>
                            </li>
                        @endif
                    @endauth
                    <li class="px-4 py-2 hover:bg-gray-700">
                        <a href="{{ route('penyaluran-bantuan.index') }}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                    ...
                    @auth
                        @if(auth()->user()->role === 'superadmin' || auth()->user()->role === 'staff')  
                    <li class="px-4 py-2 hover:bg-gray-700">
                        <a href="{{ route('penyaluran-bantuan.report') }}"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bookmarks" viewBox="0 0 16 16">
