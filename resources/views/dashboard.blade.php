...
<form method="GET" action="{{ route('dashboard') }}" class="flex flex-wrap gap-4 mb-6 items-center">
    ...
    <div class="flex flex-col w-full sm:w-1/4">
        <input type="text" name="search" placeholder="Cari Nama" value="{{ request('search') }}"
            class="p-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 w-full" />
    </div>

    <!-- Filter Kelas (Lebar 5cm atau 50px) -->
    <div class="flex flex-col w-full sm:w-1/6">
        <select name="kelas" class="p-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 w-full">
    ...
    <div class="flex flex-col w-full sm:w-1/6">
        <select name="he_qi" class="p-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-300 w-full">
            <option value="">Pilih He Qi</option>
            <option value="Pusat" {{ request('he_qi') == 'Pusat' ? 'selected' : '' }}>Pusat</option>
            <option value="Barat" {{ request('he_qi') == 'Barat' ? 'selected' : '' }}>Barat</option>
            <option value="Utara" {{ request('he_qi') == 'Utara' ? 'selected' : '' }}>Utara</option>
            <option value="Timur" {{ request('he_qi') == 'Timur' ? 'selected' : '' }}>Timur</option>
        </select>
    </div>

    <!-- Filter Sekolah -->
    <div class="flex flex-col w-full sm:w-1/4">
        <select name="sekolah" class="p-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400 w-full">
            ...
            <tbody class="divide-y divide-gray-200 text-gray-800">
                @foreach($siswas as $siswa)
                    <tr class="hover:bg-gray-100">
                        ...
                        <td class="px-6 py-4">
                            @if(in_array(auth()->user()->role, ['staff', 'superadmin']))
                                <a href="{{ route('catatan-anak.create', ['siswaId' => $siswa->id]) }}"
